...
namespace lm {
  ...
  Renderer::Renderer(Window& window, Camera& camera) : m_window(window), m_camera(camera) {
    ...
    m_gbuffer_albedo = Texture{CONSTANTS::RENDER_WIDTH, CONSTANTS::RENDER_HEIGHT, bgfx::TextureFormat::RGBA16F, 0 | BGFX_TEXTURE_RT | BGFX_SAMPLER_POINT};
    ...
    bgfx::setViewClear(LIGHT_VIEW, BGFX_CLEAR_COLOR);
  ...
  void Renderer::end_frame() {
    ...
    m_upscale_program.set_uniform("u_camera_shift",
                                  glm::vec4(m_camera.get_shift() + uv_one_pixel, 1.0f - (2.0f * uv_one_pixel.x), 1.0f - (2.0f * uv_one_pixel.y)));
    ...
    // TODO
    bgfx::blit(UPLOAD_VIEW, m_cpu_side_id_texture, 0, 0, m_gbuffer_data);
